var Designer = function (options, $) {
    ...
    var operationsSelector = options.operations || "#operations",
        ...
        zctx = zoom.length > 0 ? zoom[0].getContext("2d") : null,
        rootPath = options.rootPath || "/",
        prefix = options.prefix || "Nutshell";
    ...
    function serialize(program) {
        return JSON.stringify(program, function (key, value) {
            ...
            if (window.Font && Font.isFont(value)) return value.name;
            if (window.Color && Color.isColor(value)) return value.toHex();
            if (window.Icon && Icon.isIcon(value)) return value.name;
    ...
    function loadProgramLocal(fileName) {
        return loadProgram(localStorage[prefix + ".File:" + fileName]);
    }
    function saveProgramLocal(fileName, prg, cnv) {
        localStorage[prefix + ".File:" + fileName] = serialize(prg);
        localStorage[prefix + ".Date:" + fileName] = new Date();
        if (cnv) {
            localStorage[prefix + ".Screenshot:" + fileName] =
                cnv.toDataURL();
        }
    }
    ...
    function updateAll(program) {
        ...
        localStorage[prefix + ".__AutoSave"] = serialize(program);
    }

    // Definition of the editor's UI itself
    return function (program) {
        // Load autosaved program if no program was passed in
        program = program || deserialize(localStorage[prefix + ".__AutoSave"] || "[]");
        ...
        persistenceToolbar.find(".load")
            .click(function () {
                ...
                for (var i = 0; i < localStorage.length; i++) {
                    ...
                    if (key.substr(0, 14) == prefix + ".File:") {
                        var fileName = key.substr(14);
                        localFiles.push({
                            fileName: fileName,
                            dateModified: new Date(
                                localStorage[prefix + ".Date:" + fileName])
                        });
                    }
                }
                                ...
                                .append(
                                    $("<a></a>")
                                        ...
                                        .append($("<img/>")
                                            ...
                                            .attr({
                                                src: localStorage[prefix + ".Screenshot:" + file.fileName],
                                                alt: file.fileName
                                            })
                                        )
